...
@bot.command()
async def rect(ctx, about = "募集\n", cnt = 4, settime = 10.0 * 60):
    cnt, settime, now, end = int(cnt), float(settime * 60), datetime.datetime.now().strftime('%Y/%m/%d %H:%M:%S'), datetime.datetime.now()+settime
    reaction_member = [">>>"]
    test = discord.Embed(title=about,colour=0x1e90ff)
    test.add_field(name=f"あと{cnt}人 募集中\n", value=None, inline=True)
    test.add_field(name=f"(募集開始：{now})\n", value=None, inline=True)
    test.add_field(name=f"(募集終了：{end})\n", value=None, inline=True)
